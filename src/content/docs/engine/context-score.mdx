...
<Aside type="note">
The score is computed in the [engine](/engine/architecture/), never in the UI. Every shell reads the same number via [`prevail score <domain> --json`](/engine/json-api/#prevail-score). The wire shape is [`ContextScore`](https://github.com/fru-dev3/prevail/blob/main/docs/schemas/ContextScore.json).
</Aside>
